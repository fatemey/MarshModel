...
for j = 1 : n
    ...
    for i = 1 : k
        ...
        C_o = Co_(j);    % ocean concertation (kg/m3)
        ...
        %-------------- Basin properties
        b_fm = bfm_(i)*10^3; % total basin width (both sides of the channel) (m)
...
hold on
for i = 1 : k
    plot(Co_*1000,unique(Sol(i,1,:)),'Linewidth',2)
end
...
hold on
for i = 1 : k
    plot(Co_*1000,unique(Sol(i,2,:)),'Linewidth',2)
end
plot([0,100],[.7,.7],'k--')
text(90,.72,'MSL')
xlabel('Concentration (mg/l)')
...
hold on
for i = 1 : k
    plot(Co_*1000,unique(Sol(i,3,:)),'Linewidth',2)
end
plot([0,100],[.7,.7],'k--')
text(90,.72,'MSL')
xlabel('Concentration (mg/l)')
...
hold on
for i = 1 : k
    plot(round(R*bfm_*10^3*L_E/Q_f,4),unique(Sol(i,1,:))./bfm_,'Linewidth',2);
end
